...
* [Les données de foudre sont-elles dipsonibles?](#les-donnees-de-foudre-sont-elles-disponibles)
...
Les utilisateurs du service [AMQP](../msc-datamart/amqp_fr.md) sont censés exécuter un démon qui télécharge des données en permanence, comme celui fourni par [Sarracenia](https://github.com/MetPX/sarracenia/blob/master/doc/fr/sarra.rst). Dans le langage AMQP, une pompe, à savoir un hôte exécutant Sarracenia, est un [courtier](https://github.com/MetPX/sarracenia/blob/master/doc/fr/Install.rst). Le courtier a une capacité limitée à mettre les produits en file d'attente lorsqu'un utilisateur est victime d'une longue interruption involontaire, par exemple de quelques heures à quelques jours. 
...
Il arrive parfois que les données de modèle de prévision numérique du temps soient en retard ou même absentes de notre serveur HTTPS de données.
...
## Peut-on avoir un fil de données de modèles de prévision numérique du temps?
...
Merci de [nous contacter](ec.dps-client.ec@canada.ca) si vous êtes intéressés par ce service.
...
Cela étant dit, l'utilitaire [wget](https://www.gnu.org/software/wget/wget) possède des fonctionnalités qui permettent de télécharger seulement certains fichiers. Par exemple, les utilisateurs peuvent rassembler leurs directives de téléchargement dans un fichier texte et l'utiliser comme argument pour la commande "wget -i list.txt".

La vitesse de téléchargement sera la même que si tous les fichiers GRIB étaient dans un seul fichier zip.

## Qu'est-ce que le système de coordonnées de référence ou datum d'un modèle de prévision numérique du temps?

Pour connaitre le système de coordonnées de référence d’un modèle de prévision numérique du temps et la projection utilisée pour produire nos fichiers de prévision numérique du temps, il suffit d’utiliser les outils [GDAL]( https://www.gdal.org/GDAL) (logiciel libre).

La commande suivante vous fournira le référentiel et la projection du fichier :
`gdalinfo -proj4 file.grib2`

En sortie, l'information commence par "The coordinates system is :".
...
Pour un modèle de prévision numérique du temps dont les données sont disponibles en format GRIB2, le paramètre `LAND_SFC` (Couverture terrestre) de l'heure de prévision 00h pour être utilisé déterminer ce masque.
...
Le produit `SNODxSDENxfacteur` pour les unités fournit probablement la meilleure estimation actuellement disponible pour l'équivalent en eau de la neige. 
...
Les [fichiers sommaires provinciaux](../msc-data/obs_station/readme_obs_insitu_xmldatamart_fr.md) sont mis à jour 6 fois par heure, mais le simple fait que nous les mettions à jour ne signifie pas nécessairement qu'une des stations du fichier sommaire a mis à jour ses données depuis la dernière exécution.
...
Actuellement, seule la carte canadienne des dangers de la foudre est disponible sur le site : https://meteo.gc.ca/lightning/. Veuillez noter que vous devez mettre à jour manuellement le site web. Il ne se met pas à jour tout seul, contrairement au site des images radar.

Les données ne sont pas encore disponibles parmi l'offre de données ouvertes, mais le seront prochainement, veuillez surveiller nos listes de diffusion [dd_info](https://lists.ec.gc.ca/cgi-bin/mailman/listinfo/dd_info) et [geomet-info](http://lists.ec.gc.ca/cgi-bin/mailman/listinfo/geomet-info) pour en être informé.

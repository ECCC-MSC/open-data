...
async function getRadarStartEndTime() {
  let response = await fetch('https://geo.weather.gc.ca/geomet/?lang=en&service=WMS&request=GetCapabilities&version=1.3.0&LAYERS=GEWPS_25km_SignificantWaveHeight_01&t=' + new Date().getTime())
...
let layers = [
    ...
    new ol.layer.Image({
      source: new ol.source.ImageWMS({
        ...
        params: {'LAYERS': 'GEWPS_25km_SignificantWaveHeight_01', 'TILED': true},
...
let map = new ol.Map({
  ...
  layers: layers,
  view: new ol.View({
    center: ol.proj.fromLonLat([-94, 51]),
    zoom: 3
  })
});

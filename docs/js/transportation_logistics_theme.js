...
// Async function used to retrieve start, end, and default time from HRDPS.CONTINENTAL_WGX layer GetCapabilities document
async function getRadarStartEndTime() {
  let response = await fetch('https://geo.weather.gc.ca/geomet/?lang=en&service=WMS&request=GetCapabilities&version=1.3.0&HRDPS.CONTINENTAL_WGX&t=' + new Date().getTime())
...
let map = new ol.Map({
  ...
  layers: layers,
  view: new ol.View({
    center: ol.proj.fromLonLat([-98, 55]),
    zoom: 3.3
  })
});
...
//   var selectElement = document.getElementById('legend-select');
//   var imageElement = document.getElementById('legend-img');

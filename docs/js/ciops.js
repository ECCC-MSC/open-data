...
// Async function used to retrieve start, end, and default time from CIOPS-East_2km_SeaWaterSalinity_0.5m layer GetCapabilities document
async function getRadarStartEndTime() {
  let response = await fetch('https://geo.weather.gc.ca/geomet/?lang=en&service=WMS&request=GetCapabilities&version=1.3.0&LAYERS=CIOPS-East_2km_SeaWaterSalinity_0.5m&t=' + new Date().getTime())
...
let map = new ol.Map({
  ...
  layers: layers,
  view: new ol.View({
    center: ol.proj.fromLonLat([-59, 45]),
    zoom: 4.2
  })
});

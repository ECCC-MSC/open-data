...
// Async function used to retrieve start, end, and default time from RAQDPS-FW.CE_PM2.5-DIFF-YAvg layer GetCapabilities document
async function getRadarStartEndTime() {
  let response = await fetch('https://geo.weather.gc.ca/geomet/?lang=en&service=WMS&request=GetCapabilities&version=1.3.0&LAYERS=RAQDPS-FW.CE_PM2.5-DIFF-YAvg&t=' + new Date().getTime())
...
// RAQDPS-FW.CE_PM2.5-DIFF-YAvg is a yearly product (dimension period P1Y),
// so the animation controls step one calendar year at a time.
...
let layers = [
    ...
    new ol.layer.Image({
      source: new ol.source.ImageWMS({
        ...
        params: {'LAYERS': 'RAQDPS-FW.CE_PM2.5-DIFF-YAvg', 'TILED': true},
...
// Displaying current map year
function updateInfo() {
  let el = document.getElementById('info');
  el.innerHTML = `Year/Année: ${currentTime.getUTCFullYear()}`
}
...
function setTime() {
  if (currentTime >= endTime) {
    ...
    currentTime.setUTCFullYear(currentTime.getUTCFullYear() + 1);
  }
  updateLayers();
  updateInfo();
}

...
// Async function used to retrieve start, end, and default time from HREPA.6P_2.5km_PCT25 layer GetCapabilities document
async function getRadarStartEndTime() {
  let response = await fetch('https://geo.weather.gc.ca/geomet/?lang=en&service=WMS&request=GetCapabilities&version=1.3.0&LAYERS=HREPA.6P_2.5km_PCT25&t=' + new Date().getTime())
...
function setTime() {
  if (currentTime >= endTime) {
    ...
    currentTime.setUTCMinutes(currentTime.getUTCMinutes() + 360);
  }
  updateLayers();
  updateInfo();
}

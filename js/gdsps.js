...
// Async function used to retrieve start, end, and default time from GDSPS_15km_StormSurge layer GetCapabilities document
async function getRadarStartEndTime() {
  let response = await fetch('https://geo.weather.gc.ca/geomet/?lang=en&service=WMS&request=GetCapabilities&version=1.3.0&LAYERS=GDSPS_15km_StormSurge&t=' + new Date().getTime())
...
let map = new ol.Map({
  ...
  layers: layers,
  view: new ol.View({
    center: ol.proj.fromLonLat([-97, 61]),
    zoom: 3
  })
});

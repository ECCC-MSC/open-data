...
## Web Map Service (WMS)
Web Map Service (WMS) requests enable a client to retrieve geospatial
...
GetFeatureInfo requests. Date and times must adhere to the ISO-8601
...
an external Style Layer Descriptor (SLD) document in the `SLD` parameter 
or by entering the SLD document directly into the `SLD_BODY` parameter.
...
* [GetCoverage for RDPA-ARC PR](geo.weather.gc.ca/geomet-beta/?SE